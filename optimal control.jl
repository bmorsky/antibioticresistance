...
function f(du,u,a,t)
  du[1] = d*u[1]*(1+(u[1]+u[2])/K-m/d-(b/d)*u[2]/(u[1]+u[2]))*u[5] + w*u[1]*(1-(u[1]+u[2])/K)*(1-u[5])
  du[2] = (wp*u[2]*(1-(u[1]+u[2])/K)+m*u[1]+b*u[1]*u[2]/(u[1]+u[2]))*u[5] + (w+s)*u[2]*(1-(u[1]+u[2])/K)*(1-u[5])
...
function condition(u,t,integrator) # Event when event_f(u,t) == 0
  if sign(0.5 - u[5])*(-1.5*u[1]*u[3] + 1.4*u[2]*u[4]) < 0
	  1
  else
	  0
  end
...
function affect!(integrator)
  ...
  integrator.u[5] = 0.5*(sign(0.5 - integrator.u[5]) + 1)
  #integrator.u[2] = -integrator.u[2]
end
