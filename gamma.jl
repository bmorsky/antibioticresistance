...
for γ = 0.5e-5:0.25e-6:2e-5
	...
	for sim = 1:num_sims
		...
		output[count,:] = [γ sum(sol[1:2,:])/(10*fin_time+1) 0]
		global count += 1
	end
	...
	for sim = 1:num_sims
		...
		output[count,:] = [γ sum(sol[1:2,:])/(10*fin_time+1) 1]
		global count += 1
	end
end
...
labs(x = bquote(gamma~(10^{-5})), y = bquote("Bacterial load"~(10^4))) +
...
scale_y_continuous(breaks=c(0,10000,20000,30000,40000),labels=c(0,1,2,3,4),expand = c(0, 0),limits=c(0,40100)) +
scale_x_continuous(breaks=c(0.000005,0.00001,0.000015,0.00002),labels=c(0.5,1,1.5,2),expand = c(0,0),limits=c(0.000005,0.0000201))

save_plot(output,filename="/home/bmorsky/antibiotic/results/gamma.png",base_height = 2.5,base_width = 8)
save(output,file="/home/bmorsky/antibiotic/results/gamma.Rda")
"""
#theme(legend.title=element_blank(),legend.position=c(.45,.7)) +
