...
# Parameters
const ξ = -0.2 # stochasticity ξ

const α = 1.6 # death rate from antibiotic
const bx = 1.4+ξ # susceptible birth rate
const by = 1.2+ξ # resistant birth rate
const d = 0.4+ξ # death rate
...
output_alwayson = 1
using RCall
@rput output_mean myxi output_alwayson maxSwitching
R"""
library(ggplot2)
...
save_plot(p_mean,filename=paste(paste("xi",myxi,sep="_"),".png", sep=""),base_height = 4,base_width = 4)
save(output_mean,file=paste(paste("xi",myxi,sep="_"),".Rda", sep=""))
"""
