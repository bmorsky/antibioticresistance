...
LV_model = @reaction_network begin
	...
	μ, Y → X # mutation
	λ, A → 0
end b d γxx γxy γyx γyy α μ λ m
...
output_alwayson = 0
for sim = 1:num_sims
	prob = SDEProblem(LV_model,[X₀; 0; A₀],(0.0,T),[b, d, γxx, γxy, γyx, γyy, α, μ, λ, m])
...
output_alwayson = 1
using RCall
@rput output_mean mykappa output_alwayson maxSwitching
R"""
library(ggplot2)
...
save_plot(p_mean,filename=paste(paste("c0_kappa",mykappa,sep="_"),".png", sep=""),base_height = 4,base_width = 4)
save(output_mean,file=paste(paste("c0_kappa",mykappa,sep="_"),".Rda", sep=""))
"""
