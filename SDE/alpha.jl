...
function ensemPulse!(α)
	for sim = 1:num_sims
		global output[count,:] = [α runPulse(α) 1];
		global count = count + 1;
	end
end

function ensemConst!(α)
	for sim = 1:num_sims
 		global output[count,:] = [α runConst(α) 0];
		global count = count + 1;
	end
end
...
scale_x_continuous(breaks=c(1.2,1.4,1.6,1.8,2,2.2,2.4),expand = c(0.005,0)) +
...
save_plot(output,filename="alpha.png",base_height = 2.5,base_width = 8)
"""
