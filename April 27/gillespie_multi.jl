...
for m = 1:5
	...
	global output1 = vcat(output1,[sol1[1,:] sol1.t[:] ones(length(sol1[1,:]),1)*(2*m-1); sol1[2,:] sol1.t[:] ones(length(sol1[2,:]),1)*2*m]) #output
	global output2 = vcat(output2,[sol2[1,:] sol2.t[:] ones(length(sol2[1,:]),1)*(2*m-1); sol2[2,:] sol2.t[:] ones(length(sol2[2,:]),1)*2*m]) #output
end
...
output1 <- ts + geom_line(data=output1,aes(x=Time,y=Number,group=Type),size=1) + ggtitle("On/off protocol")+ scale_color_manual(values=c("black","blue","red","blue","red","blue","red","blue","red","blue","red","blue","red","blue","red","blue","red","blue","red","blue","red"),labels = c("Susceptible", "Resistant")) + aes(color = factor(Type)) + theme(legend.title=element_blank(),legend.position=c(.65,.55)) + labs(x = "Time", y = "Bacterial load") + scale_y_continuous(expand = c(0, 0)) + scale_x_continuous(limits = c(0,2050), expand = c(0,0))
output2 <- ts + geom_line(data=output2,aes(x=Time,y=Number,group=Type),size=1) + ggtitle("Constant application")+ scale_color_manual(values=c("black","blue","red","blue","red","blue","red","blue","red","blue","red","blue","red","blue","red","blue","red","blue","red","blue","red"),labels = c("Susceptible", "Resistant")) + aes(color = factor(Type)) + theme(legend.title=element_blank(),legend.position=c(.65,.55)) + labs(x = "Time", y = "Bacterial load") + scale_y_continuous(expand = c(0, 0)) + scale_x_continuous(limits = c(0,2050), expand = c(0,0))

plot_out <- plot_grid(output1,output2,labels=letters[1:2],ncol=2)
save(best_protocol,file="~/Desktop/gillespie_ex.Rda")
save_plot(plot_out,filename="~/Documents/Notre Dame/ND paper 2/Code/ts_protocol_vs_constant_stochastic_multi.png",base_height = 5,base_width = 10)
"""
