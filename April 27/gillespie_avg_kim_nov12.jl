...
num_sims = 2
antibiotic_amount = 100 # initial input rate of bacteria
time = 2 # length of process
...
α₁ = 0.08 # susceptible death rate from antibiotic
α₂ = 0.015 # resistant death rate from antibiotic
	...
	for durOn = step_length:step_length:step_length*(num_steps-1)
		for durOff = step_length:step_length:step_length*num_steps-durOn
			...
			function affectOn!(integrator) # effect that occurs when the condition is met
				#integrator.u[3] += 200
				integrator.u[3] = antibiotic_amount
			end
