...
for μ = 1e-6:2e-6:1e-4
	...
	for sim = 1:num_sims
		...
		output[count,:] = [μ sum(sol[1:2,:])/(10*fin_time+1) 0]
		global count += 1
	end
	...
	for sim = 1:num_sims
		...
		output[count,:] = [μ sum(sol[1:2,:])/(10*fin_time+1) 1]
		global count += 1
	end
end
...
labs(x = bquote(mu~(10^{-5})), y = bquote("Bacterial load"~(10^4))) +
...
scale_x_continuous(breaks=c(0.000001,0.00005,0.0001),labels=c(0.1,5,10),expand = c(0,0),limits=c(0.000001,0.000101))

save_plot(output,filename="/home/bmorsky/antibiotic/results/mu.png",base_height = 2.5,base_width = 8)
save(output,file="/home/bmorsky/antibiotic/results/mu.Rda")
"""
#theme(legend.title=element_blank(),legend.position=c(.45,.5)) +
