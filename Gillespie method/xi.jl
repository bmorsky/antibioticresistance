...
for ξ = -0.05:0.005:0.25
	...
	for sim = 1:num_sims
		...
		output[count,:] = [ξ sum(sol[1:2,:])/(10*fin_time+1) 0]
		global count += 1
	end
	...
	for sim = 1:num_sims
		...
		output[count,:] = [ξ sum(sol[1:2,:])/(10*fin_time+1) 1]
		global count += 1
	end
end
...
labs(x = bquote(xi), y = bquote("Bacterial load"~(10^4))) +
...
scale_x_continuous(breaks=c(-0.05,0.1,0.25),expand = c(0,0),limits=c(-0.05,0.253))

save_plot(output,filename="/home/bmorsky/antibiotic/results/xi.png",base_height = 2.5,base_width = 8)
save(output,file="/home/bmorsky/antibiotic/results/xi.Rda")
"""
