...
for κ = 1:0.25:20
	...
	for sim = 1:num_sims
		...
		output[count,:] = [κ sum(sol[1:2,:])/(10*fin_time+1) 0]
		global count += 1
	end
	...
	for sim = 1:num_sims
		...
		output[count,:] = [κ sum(sol[1:2,:])/(10*fin_time+1) 1]
		global count += 1
	end
end
...
labs(x = bquote(kappa), y = bquote("Bacterial load"~(10^4))) +
...
scale_x_continuous(breaks=c(1,5,10,15,20),expand = c(0,0),limits=c(1,20.1))

save_plot(output,filename="/home/bmorsky/antibiotic/results/kappa.png",base_height = 2.5,base_width = 8)
save(output,file="/home/bmorsky/antibiotic/results/kappa.Rda")
"""
